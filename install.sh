...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/curzes/.dot.git"
[ -z "$dotfilesrepossh" ] && dotfilesrepossh="git@github.com:curzes/.dot.git"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/curzes/scripts/master/progs.csv"
...
adduserandpass() { \
	...
	useradd -m -G wheel,uucp,lock,adbusers -s /bin/zsh "$name" >/dev/null 2>&1 ||
	usermod -a -G wheel,uucp,lock,adbusers "$name" && mkdir -p /home/"$name" && chown "$name":wheel /home/"$name"
